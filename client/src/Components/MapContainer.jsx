...
class MapContainer extends Component {
  ...
  render() {
    ...
    const addresses = events.filter(event => event.address).map(event => {
      const arr = event.address.split(',');
      const address = arr[0].trim();
      return address;
    });
    const cities = events.filter(event => event.address).map(event => {
      const arr = event.address.split(',');
      const city = arr[1].trim();
      return city;
    });
    const states = events.filter(event => event.address).map(event => {
      const arr = event.address.split(',');
      const state = arr[arr.length - 1].trim().slice(0, 2);
      return state;
    });
    const categories = events.filter(event => event.category).map(event => event.category);
